import math
from thymio_utils import BUTTON_CENTER, MOTOR_LEFT, MOTOR_RIGHT, PROXIMITY_GROUND_DELTA, PROXIMITY_GROUND_REFLECTED, SingleSerialThymioRunner, ThymioObserver
...
class LineFollower(ThymioObserver):
	...
	def exponential_curve(self, steps:int, stop:int, idklol:float):
	...
	def handle_buttons(self):
		if self.th[BUTTON_CENTER]:
			...
			self.stop()
		if self.th["button.right"]:
			self.speed = self.speed + self.speed_step
			print(self.speed)
		if self.th["button.left"]:
			self.speed = self.speed - self.speed_step
			print(self.speed)
